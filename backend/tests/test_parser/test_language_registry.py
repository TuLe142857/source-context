...
from app.parser import LanguageRegistry, UASTConverter
...
class TestLoadConfig:
    ...
    @pytest.mark.parametrize(["language_name", "file_names"], [("python", ["test.py"]), ("java", ["test.java"])])
    def test_get_language_object(self, registry: LanguageRegistry, language_name: str, file_names: list[str]):
    ...
    @pytest.mark.parametrize(["language_name", "file_names"], [("python", ["test.py"]), ("java", ["test.java"])])
    def test_get_parser_object(self, registry: LanguageRegistry, language_name: str, file_names: list[str]):
    ...
    @pytest.mark.parametrize(["language_name", "file_names"], [("python", ["test.py"]), ("java", ["test.java"])])
    def test_get_converter_object(self, registry: LanguageRegistry, language_name: str, file_names: list[str]):
