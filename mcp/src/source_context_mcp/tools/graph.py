...
def register_tools(mcp: MCPServer) -> None:
    ...
    @mcp.tool(description="Get node information: id, name, file_id, position in file")
    async def get_node_info(client: ApiClientDep, node_id: str) -> Any:
        res = await client.get(f"/graph/nodes/{node_id}")
        return res.result()

    @mcp.tool(description="Get node information.")
    async def get_node_content(client: ApiClientDep, node_id: str) -> Any:
